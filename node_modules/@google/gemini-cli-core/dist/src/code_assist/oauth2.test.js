...
import { getOauthClient, resetOauthClientForTesting, clearCachedCredentialFile, clearOauthClientCache, authEvents, } from './oauth2.js';
...
describe('oauth2', () => {
    ...
    describe('with encrypted flag false', () => {
        ...
        it('should perform login with user code', async () => {
            ...
            expect(vi.mocked(writeToStdout)).toHaveBeenCalledWith(expect.stringContaining(mockAuthUrl));
        ...
        it('should cache Google Account when logging in with user code', async () => {
            ...
            vi.mocked(OAuth2Client).mockImplementation(() => mockOAuth2Client);
            vi.spyOn(crypto, 'randomBytes').mockReturnValue('test-state');
            const mockReadline = {
                question: vi.fn((_query, callback) => callback(mockCode)),
                close: vi.fn(),
                on: vi.fn(),
        ...
        describe('cancellation', () => {
            ...
            it('should cancel when Ctrl+C (0x03) is received on stdin', async () => {
                ...
                const stdinRemoveListenerSpy = vi.spyOn(process.stdin, 'removeListener');
                const clientPromise = getOauthClient(AuthType.LOGIN_WITH_GOOGLE, mockConfig);
                // Wait for the stdin handler to be registered
                let dataHandler;
                await vi.waitFor(() => {
                    const dataCall = stdinOnSpy.mock.calls.find((call) => call[0] === 'data');
                    dataHandler = dataCall?.[1];
                    if (!dataHandler)
                        throw new Error('stdin handler not registered yet');
                });
                ...
                await expect(clientPromise).rejects.toThrow(FatalCancellationError);
                expect(stdinRemoveListenerSpy).toHaveBeenCalledWith('data', expect.any(Function));
            ...
            it('should throw FatalCancellationError when consent is denied', async () => {
                ...
                await expect(getOauthClient(AuthType.LOGIN_WITH_GOOGLE, mockConfig)).rejects.toThrow(FatalCancellationError);
            });
        });
    ...
    describe('with encrypted flag true', () => {
        ...
        it('should save credentials using OAuthCredentialStorage during web login', async () => {
            ...
            requestCallback(mockReq, mockRes);
            await clientPromise;
            expect(vi.mocked(OAuthCredentialStorage.saveCredentials)).toHaveBeenCalledWith(mockTokens);
            const credsPath = path.join(tempHomeDir, GEMINI_DIR, 'oauth_creds.json');
            expect(fs.existsSync(credsPath)).toBe(false);
        });
        it('should load credentials using OAuthCredentialStorage and not from file', async () => {
            ...
            vi.mocked(OAuth2Client).mockImplementation(() => mockClient);
            await getOauthClient(AuthType.LOGIN_WITH_GOOGLE, mockConfig);
            expect(vi.mocked(OAuthCredentialStorage.loadCredentials)).toHaveBeenCalled();
            expect(mockClient.setCredentials).toHaveBeenCalledWith(cachedCreds);
