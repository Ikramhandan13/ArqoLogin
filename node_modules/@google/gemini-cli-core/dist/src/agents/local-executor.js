...
import { AgentTerminateMode, DEFAULT_QUERY_STRING, DEFAULT_MAX_TURNS, DEFAULT_MAX_TIME_MINUTES, } from './types.js';
...
export class LocalAgentExecutor {
    ...
    onWaitingForConfirmation) {
        ...
        const { nextMessage, submittedOutput, taskCompleted } = await this.processFunctionCalls(functionCalls, combinedSignal, promptId, onWaitingForConfirmation);
        if (taskCompleted) {
     ...
     */
    async executeFinalWarningTurn(chat, turnCounter, reason, externalSignal, // The original signal passed to run()
    onWaitingForConfirmation) {
    ...
    async run(inputs, signal) {
        ...
        try {
            ...
            if (terminateReason !== AgentTerminateMode.ERROR &&
                ...
                terminateReason, signal, // Pass the external signal
                onWaitingForConfirmation);
            ...
            if (error instanceof Error &&
                ...
                if (chat && tools) {
                    ...
                    AgentTerminateMode.TIMEOUT, signal, onWaitingForConfirmation);
                    if (recoveryResult !== null) {
                        // Recovery Succeeded
                        terminateReason = AgentTerminateMode.GOAL;
                        finalResult = recoveryResult;
