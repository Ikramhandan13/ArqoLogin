 ...
 */
import { CoreToolCallStatus, AuthType, EditTool, GeminiClient, ToolConfirmationOutcome, ToolErrorType, ToolRegistry, } from '../index.js';
...
describe('loggers', () => {
    ...
    describe('logApiResponse', () => {
        ...
        it('should log an API response with a role', () => {
            const event = new ApiResponseEvent('test-model', 100, { prompt_id: 'prompt-id-role', contents: [] }, { candidates: [] }, AuthType.LOGIN_WITH_GOOGLE, {}, 'test-response', LlmRole.SUBAGENT);
            logApiResponse(mockConfig, event);
            expect(mockLogger.emit).toHaveBeenCalledWith({
                body: 'API response from test-model. Status: 200. Duration: 100ms.',
                attributes: expect.objectContaining({
    ...
    describe('logApiError', () => {
        ...
        it('should log an API error with a role', () => {
            const event = new ApiErrorEvent('test-model', 'error', 100, { prompt_id: 'prompt-id-role', contents: [] }, AuthType.LOGIN_WITH_GOOGLE, 'ApiError', 503, LlmRole.SUBAGENT);
            logApiError(mockConfig, event);
            expect(mockLogger.emit).toHaveBeenCalledWith({
    ...
    describe('logApiRequest', () => {
        ...
        it('should log an API request with a role', () => {
            const event = new ApiRequestEvent('test-model', { prompt_id: 'prompt-id-role', contents: [] }, 'request text', LlmRole.SUBAGENT);
            logApiRequest(mockConfig, event);
            expect(mockLogger.emit).toHaveBeenCalledWith({
                body: 'API request to test-model.',
                attributes: expect.objectContaining({
    ...
    describe('logToolCall', () => {
        ...
        it('should log a tool call with mcp_server_name for MCP tools', () => {
            const mockMcpTool = new DiscoveredMCPTool({}, 'mock_mcp_server', 'mock_mcp_tool', 'tool description', {
                ...
                required: ['arg1', 'arg2'],
            }, createMockMessageBus(), false, undefined, undefined, undefined, 'test-extension', 'test-extension-id');
            const call = {
                status: CoreToolCallStatus.Success,
                request: {
                    name: 'mock_mcp_tool',
    ...
    describe('logModelRouting', () => {
        ...
        it('should log the event with numerical routing fields', () => {
            const event = new ModelRoutingEvent('gemini-pro', 'NumericalClassifier (Strict)', 150, '[Score: 90 / Threshold: 80] reasoning', false, undefined, true, '80');
            logModelRouting(mockConfig, event);
            expect(ClearcutLogger.prototype.logModelRoutingEvent).toHaveBeenCalledWith(event);
            expect(mockLogger.emit).toHaveBeenCalledWith({
    ...
    describe('logExtensionInstall', () => {
        ...
        it('should log extension install event', async () => {
            const event = new ExtensionInstallEvent('testing', 'testing-hash', 'testing-id', '0.1.0', 'git', CoreToolCallStatus.Success);
    ...
    describe('logExtensionUpdate', async () => {
        ...
        it('should log extension update event', async () => {
            const event = new ExtensionUpdateEvent('testing', 'testing-hash', 'testing-id', '0.1.0', '0.1.1', 'git', CoreToolCallStatus.Success);
    ...
    describe('logExtensionUninstall', async () => {
        ...
        it('should log extension uninstall event', async () => {
            const event = new ExtensionUninstallEvent('testing', 'testing-hash', 'testing-id', CoreToolCallStatus.Success);
