...
import { PolicyDecision, InProcessCheckerType, ApprovalMode, PRIORITY_SUBAGENT_TOOL, } from './types.js';
...
vi.mock('../tools/tool-names.js', async (importOriginal) => {
    ...
    return {
        ...
        getToolAliases: vi.fn().mockImplementation((name) => {
...
describe('PolicyEngine', () => {
    ...
    describe('shell command parsing failure', () => {
        it('should return ALLOW in YOLO mode even if shell command parsing fails', async () => {
            ...
            vi.mocked(splitCommands).mockReturnValueOnce([]);
            const result = await engine.check({ name: 'run_shell_command', args: { command: 'complex command' } }, undefined);
            expect(result.decision).toBe(PolicyDecision.ALLOW);
    ...
    describe('YOLO mode with ask_user tool', () => {
        it('should return ASK_USER for ask_user tool even in YOLO mode', async () => {
            ...
            const result = await engine.check({ name: 'ask_user', args: {} }, undefined);
            expect(result.decision).toBe(PolicyDecision.ASK_USER);
        });
        it('should return ALLOW for other tools in YOLO mode', async () => {
            ...
            const result = await engine.check({ name: 'run_shell_command', args: { command: 'ls' } }, undefined);
            expect(result.decision).toBe(PolicyDecision.ALLOW);
        });
    });
    describe('Plan Mode', () => {
        it('should allow activate_skill but deny shell commands in Plan Mode', async () => {
            ...
            const skillResult = await engine.check({ name: 'activate_skill', args: { name: 'test' } }, undefined);
            expect(skillResult.decision).toBe(PolicyDecision.ALLOW);
            const shellResult = await engine.check({ name: 'run_shell_command', args: { command: 'ls' } }, undefined);
