...
describe('Turn', () => {
    ...
    describe('run', () => {
        it('should yield content events for text parts', async () => {
            ...
            expect(mockSendMessageStream).toHaveBeenCalledWith({ model: 'gemini' }, reqParts, 'prompt-id-1', expect.any(AbortSignal), LlmRole.MAIN, undefined);
        ...
        it('should handle function calls with undefined name or args', async () => {
            ...
            mockSendMessageStream.mockResolvedValue(mockResponseStream);
            const events = [];
            for await (const event of turn.run({ model: 'gemini' }, [{ text: 'Test undefined tool parts' }], new AbortController().signal)) {
                events.push(event);
            }
        ...
        it('should yield citation and finished events when response has citationMetadata', async () => {
            ...
            mockSendMessageStream.mockResolvedValue(mockResponseStream);
            const events = [];
            for await (const event of turn.run({ model: 'gemini' }, [{ text: 'Test citations' }], new AbortController().signal)) {
                events.push(event);
            }
            expect(events).toEqual([
        ...
        it('should process all parts when thought is first part in chunk', async () => {
            ...
            mockSendMessageStream.mockResolvedValue(mockResponseStream);
            const events = [];
            for await (const event of turn.run({ model: 'gemini' }, [{ text: 'Test mixed content' }], new AbortController().signal)) {
                events.push(event);
            }
            // Should yield:
