...
import { PREVIEW_GEMINI_MODEL, PREVIEW_GEMINI_FLASH_MODEL, DEFAULT_GEMINI_MODEL_AUTO, DEFAULT_GEMINI_MODEL, DEFAULT_GEMINI_FLASH_LITE_MODEL, } from '../config/models.js';
...
describe('Core System Prompt (prompts.ts)', () => {
    ...
    it('should include available_skills with updated verbiage for preview models', () => {
        ...
        const prompt = getCoreSystemPrompt(mockConfig);
        expect(prompt).toContain('# Available Agent Skills');
        expect(prompt).toContain("To activate a skill and receive its detailed instructions, call the `activate_skill` tool with the skill's name.");
        expect(prompt).toMatchSnapshot();
    });
    ...
    it('should include sub-agents in XML for preview models', () => {
        ...
        vi.mocked(mockConfig.getAgentRegistry().getAllDefinitions).mockReturnValue(agents);
        const prompt = getCoreSystemPrompt(mockConfig);
        ...
        expect(prompt).toContain('<description>A test agent description</description>');
        expect(prompt).toContain('</subagent>');
        expect(prompt).toContain('</available_subagents>');
        expect(prompt).toMatchSnapshot();
    });
    it('should use legacy system prompt for non-preview model', () => {
        vi.mocked(mockConfig.getActiveModel).mockReturnValue(DEFAULT_GEMINI_FLASH_LITE_MODEL);
        const prompt = getCoreSystemPrompt(mockConfig);
        expect(prompt).toContain('You are an interactive CLI agent specializing in software engineering tasks.');
    ...
    it('should include mandate to distinguish between Directives and Inquiries', () => {
        ...
        expect(prompt).toContain('Assume all requests are Inquiries unless they contain an explicit instruction to perform a task.');
        expect(prompt).toMatchSnapshot();
    });
    it.each([
        ['empty string', ''],
    ...
    it('should redact grep and glob from the system prompt when they are disabled', () => {
        ...
        expect(prompt).toContain('Use search tools extensively to understand file structures, existing code patterns, and conventions.');
    });
    it.each([
        ...
        if (expectCodebaseInvestigator) {
            expect(prompt).toContain(`Utilize specialized sub-agents (e.g., \`codebase_investigator\`) as the primary mechanism for initial discovery`);
            expect(prompt).not.toContain('Use `grep_search` and `glob` search tools extensively');
        }
        else {
            expect(prompt).not.toContain(`Utilize specialized sub-agents (e.g., \`codebase_investigator\`) as the primary mechanism for initial discovery`);
            expect(prompt).toContain('Use `grep_search` and `glob` search tools extensively');
        }
        expect(prompt).toMatchSnapshot();
    });
    describe('ApprovalMode in System Prompt', () => {
        ...
        it('should include read-only MCP tools in PLAN mode', () => {
            vi.mocked(mockConfig.getApprovalMode).mockReturnValue(ApprovalMode.PLAN);
            const readOnlyMcpTool = new DiscoveredMCPTool({}, 'readonly-server', 'read_static_value', 'A read-only tool', {}, {}, false, true);
            const nonReadOnlyMcpTool = new DiscoveredMCPTool({}, 'nonreadonly-server', 'non_read_static_value', 'A non-read-only tool', {}, {}, false, false);
            ...
            expect(prompt).not.toContain('`non_read_static_value` (nonreadonly-server)');
        });
        ...
        describe('Approved Plan in Plan Mode', () => {
            beforeEach(() => {
                vi.mocked(mockConfig.getApprovalMode).mockReturnValue(ApprovalMode.PLAN);
                vi.mocked(mockConfig.storage.getProjectTempPlansDir).mockReturnValue('/tmp/plans');
            });
    ...
    describe('Platform-specific and Background Process instructions', () => {
        it('should include Windows-specific shell efficiency commands on win32', () => {
            ...
            vi.mocked(mockConfig.getActiveModel).mockReturnValue(DEFAULT_GEMINI_FLASH_LITE_MODEL);
            const prompt = getCoreSystemPrompt(mockConfig);
            expect(prompt).toContain("using commands like 'type' or 'findstr' (on CMD) and 'Get-Content' or 'Select-String' (on PowerShell)");
            expect(prompt).not.toContain("using commands like 'grep', 'tail', 'head'");
        });
        it('should include generic shell efficiency commands on non-Windows', () => {
            ...
            expect(prompt).not.toContain("using commands like 'type' or 'findstr' (on CMD) and 'Get-Content' or 'Select-String' (on PowerShell)");
        });
        it('should use is_background parameter in background process instructions', () => {
            const prompt = getCoreSystemPrompt(mockConfig);
            expect(prompt).toContain('To run a command in the background, set the `is_background` parameter to true.');
    ...
    it('should include planning phase suggestion when enter_plan_mode tool is enabled', () => {
        ...
        const prompt = getCoreSystemPrompt(mockConfig);
        expect(prompt).toContain('If the request is ambiguous, broad in scope, or involves creating a new feature/application, you MUST use the `enter_plan_mode` tool to design your approach before making changes. Do NOT use Plan Mode for straightforward bug fixes, answering questions, or simple inquiries.');
        expect(prompt).toMatchSnapshot();
    });
