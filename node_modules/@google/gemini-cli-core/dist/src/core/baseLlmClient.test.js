...
describe('BaseLlmClient', () => {
    ...
    describe('generateJson - Success Scenarios', () => {
        it('should call generateContent with correct parameters, defaults, and utilize retry mechanism', async () => {
            ...
            expect(retryWithBackoff).toHaveBeenCalledWith(expect.any(Function), expect.objectContaining({
            ...
            }, 'test-prompt-id', LlmRole.UTILITY_TOOL);
        });
        it('should include system instructions when provided', async () => {
            ...
            }), expect.any(String), LlmRole.UTILITY_TOOL);
        });
    ...
    describe('generateJson - Response Cleaning', () => {
        ...
        it('should use the resolved model name when logging malformed JSON telemetry', async () => {
            ...
            mockConfig.modelConfigService.getResolvedConfig.mockReturnValue({
                model: resolvedModel,
    ...
    describe('generateJson - Error Handling', () => {
        it('should throw and report error for empty response after retry exhaustion', async () => {
            ...
            await expect(client.generateJson(defaultOptions)).rejects.toThrow('Failed to generate content: Retry attempts exhausted for invalid content');
            // Verify error reporting details
            expect(reportError).toHaveBeenCalledTimes(1);
            expect(reportError).toHaveBeenCalledWith(expect.any(Error), 'API returned invalid content after all retries.', defaultOptions.contents, 'generateJson-invalid-content');
        });
        ...
        it('should throw and report generic API errors', async () => {
            ...
            await expect(client.generateJson(defaultOptions)).rejects.toThrow('Failed to generate content: Service Unavailable (503)');
            // Verify generic error reporting
            expect(reportError).toHaveBeenCalledTimes(1);
            expect(reportError).toHaveBeenCalledWith(apiError, 'Error generating content via API.', defaultOptions.contents, 'generateJson-api');
        });
    ...
    describe('generateContent', () => {
        it('should call generateContent with correct parameters and utilize retry mechanism', async () => {
            ...
            }, 'content-prompt-id', LlmRole.UTILITY_TOOL);
        });
        it('should validate content using shouldRetryOnContent function', async () => {
        ...
        it('should throw and report error for empty response after retry exhaustion', async () => {
            ...
            await expect(client.generateContent(options)).rejects.toThrow('Failed to generate content: Retry attempts exhausted for invalid content');
            // Verify error reporting details
            expect(reportError).toHaveBeenCalledTimes(1);
            expect(reportError).toHaveBeenCalledWith(expect.any(Error), 'API returned invalid content after all retries.', options.contents, 'generateContent-invalid-content');
        });
    });
    describe('Availability Service Integration', () => {
        ...
        beforeEach(() => {
            ...
            mockConfig.setActiveModel = vi.fn((model) => {
        ...
        it('marks the final attempted model healthy after a retry with availability enabled', async () => {
            ...
            expect(mockGenerateContent).toHaveBeenLastCalledWith(expect.objectContaining({ model: fallbackModel }), expect.any(String), LlmRole.UTILITY_TOOL);
        });
        ...
        it('should mark healthy and honor availability selection when using generateJson', async () => {
            ...
            expect(mockGenerateContent).toHaveBeenLastCalledWith(expect.objectContaining({ model: availableModel }), jsonOptions.promptId, LlmRole.UTILITY_TOOL);
        });
