...
export class GeminiClient {
    ...
    async startChat(extraHistory, resumedSessionData) {
        ...
        try {
            const systemMemory = this.config.getUserMemory();
            const systemInstruction = getCoreSystemPrompt(this.config, systemMemory);
            return new GeminiChat(this.config, systemInstruction, tools, history, resumedSessionData, async (modelId) => {
                this.lastUsedModelId = modelId;
                const toolRegistry = this.config.getToolRegistry();
                const toolDeclarations = toolRegistry.getFunctionDeclarations(modelId);
    ...
    async tryMaskToolOutputs(history) {
        ...
        const result = await this.toolOutputMaskingService.mask(history, this.config);
