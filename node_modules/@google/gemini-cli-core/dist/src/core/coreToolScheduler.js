...
export class CoreToolScheduler {
    ...
    isRunning() {
        return (this.isFinalizingToolCalls ||
            this.toolCalls.some((call) => call.status === CoreToolCallStatus.Executing ||
    ...
    cancelAll(signal) {
        ...
        if (this.toolCalls.length > 0) {
            ...
            if (activeCall.status === CoreToolCallStatus.AwaitingApproval ||
                ...
                this.setStatusInternal(activeCall.request.callId, CoreToolCallStatus.Cancelled, signal, 'User cancelled the operation.');
            }
        }
        ...
        if (toolCall.status === CoreToolCallStatus.Validating) {
            ...
            try {
                ...
                if (decision === PolicyDecision.DENY) {
                    ...
                    this.setStatusInternal(reqInfo.callId, CoreToolCallStatus.Error, signal, createErrorResponse(reqInfo, new Error(errorMessage), errorType));
                    await this.checkAndNotifyCompletion(signal);
                    return;
                }
                if (decision === PolicyDecision.ALLOW) {
                    ...
                    if (!confirmationDetails) {
                        ...
                        this.setStatusInternal(reqInfo.callId, CoreToolCallStatus.AwaitingApproval, signal, wrappedConfirmationDetails);
                    }
                }
            }
            catch (error) {
                if (signal.aborted) {
                    ...
                    this.setStatusInternal(reqInfo.callId, CoreToolCallStatus.Error, signal, createErrorResponse(reqInfo, error instanceof Error ? error : new Error(String(error)), ToolErrorType.UNHANDLED_EXCEPTION));
                    await this.checkAndNotifyCompletion(signal);
                }
            }
        }
    ...
    async attemptExecutionOfScheduledCalls(signal) {
        const allCallsFinalOrScheduled = this.toolCalls.every((call) => call.status === CoreToolCallStatus.Scheduled ||
        ...
        if (allCallsFinalOrScheduled) {
            ...
            for (const toolCall of callsToExecute) {
                if (toolCall.status !== CoreToolCallStatus.Scheduled)
                    continue;
                ...
                const completedCall = await this.toolExecutor.execute({
                    ...
                    outputUpdateHandler: (callId, output) => {
                        ...
                        this.toolCalls = this.toolCalls.map((tc) => tc.request.callId === callId &&
    ...
    async checkAndNotifyCompletion(signal) {
        ...
        if (this.toolCalls.length === 0) {
            ...
            const activeCall = this.toolCalls[0];
            const isTerminal = activeCall.status === CoreToolCallStatus.Success ||
