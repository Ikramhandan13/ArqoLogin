...
import { GEMINI_CLI_IDENTIFICATION_ENV_VAR, GEMINI_CLI_IDENTIFICATION_ENV_VAR_VALUE, } from '../services/shellExecutionService.js';
    ...
    try {
        ...
        for (const toolDef of response.tools) {
            try {
                ...
                const tool = new DiscoveredMCPTool(mcpCallableTool, mcpServerName, toolDef.name, toolDef.description ?? '', toolDef.inputSchema ?? { type: 'object', properties: {} }, messageBus, mcpServerConfig.trust, isReadOnly, undefined, cliConfig, mcpServerConfig.extension?.name, mcpServerConfig.extension?.id);
                ...
                coreEvents.emitFeedback('error', `Error discovering tool: '${toolDef.name
                // eslint-disable-next-line @typescript-eslint/no-unsafe-type-assertion
                }' from MCP server '${mcpServerName}': ${error.message}`, error);
            }
        }
...
export async function createTransport(mcpServerName, mcpServerConfig, debugMode, sanitizationConfig) {
    ...
    if (mcpServerConfig.command) {
        ...
        if (mcpServerConfig.command === 'xcrun' &&
        ...
        if (debugMode) {
            ...
            const underlyingTransport = transport instanceof XcodeMcpBridgeFixTransport
                ? // eslint-disable-next-line @typescript-eslint/no-explicit-any, @typescript-eslint/no-unsafe-type-assertion
                    transport.transport
                : transport;
            if (underlyingTransport instanceof StdioClientTransport &&
