...
export class McpClientManager {
    ...
    blockedMcpServers = [];
    constructor(clientVersion, toolRegistry, cliConfig, eventEmitter) {
    ...
    async stopExtension(extension) {
        ...
        await Promise.all(Object.keys(extension.mcpServers ?? {}).map((name) => {
            ...
            if (config?.extension?.id === extension.id) {
                ...
                const index = this.blockedMcpServers.findIndex((s) => s.name === name && s.extensionName === extension.name);
                if (index !== -1) {
    ...
    async startExtension(extension) {
        ...
        await Promise.all(Object.entries(extension.mcpServers ?? {}).map(([name, config]) => this.maybeDiscoverMcpServer(name, {
            ...config,
            extension,
        })));
        await this.cliConfig.refreshMcpContext();
    ...
    async disconnectClient(name, skipRefresh = false) {
    ...
    async maybeDiscoverMcpServer(name, config) {
        ...
        if (existing && existing.getServerConfig().extension !== config.extension) {
            const extensionText = config.extension
                ...
                : '';
            debugLogger.warn(`Skipping MCP config for server with name "${name}"${extensionText} as it already exists.`);
            return;
        }
            ...
            (async () => {
                try {
                    ...
                    const client = existing ??
                        new McpClient(name, config, this.toolRegistry, this.cliConfig.getPromptRegistry(), this.cliConfig.getResourceRegistry(), this.cliConfig.getWorkspaceContext(), this.cliConfig, this.cliConfig.getDebugMode(), this.clientVersion, async () => {
    ...
    async startConfiguredMcpServers() {
        ...
        this.eventEmitter?.emit('mcp-client-update', this.clients);
        await Promise.all(Object.entries(servers).map(([name, config]) => this.maybeDiscoverMcpServer(name, config)));
     ...
     */
    async restart() {
        await Promise.all(Array.from(this.allServerConfigs.entries()).map(async ([name, config]) => {
            try {
    ...
    getMcpInstructions() {
        ...
        for (const [name, client] of this.clients) {
            ...
            if (clientInstructions) {
                instructions.push(`The following are instructions provided by the tool server '${name}':\n---[start of server instructions]---\n${clientInstructions}\n---[end of server instructions]---`);
            }
        }
