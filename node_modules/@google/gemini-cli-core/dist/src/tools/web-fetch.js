...
`;
    ...
    async execute(signal) {
        ...
        const geminiClient = this.config.getGeminiClient();
        try {
            const response = await geminiClient.generateContent({ model: 'web-fetch' }, [{ role: 'user', parts: [{ text: userPrompt }] }], signal, // Pass signal
...
export class WebFetchTool extends BaseDeclarativeTool {
    ...
    constructor(config, messageBus) {
        super(WebFetchTool.Name, 'WebFetch', WEB_FETCH_DEFINITION.base.description, Kind.Fetch, WEB_FETCH_DEFINITION.base.parametersJsonSchema, messageBus, true, // isOutputMarkdown
        false);
        this.config = config;
    }
