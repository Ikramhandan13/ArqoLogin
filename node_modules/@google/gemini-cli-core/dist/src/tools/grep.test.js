...
describe('GrepTool', () => {
    ...
    describe('execute', () => {
        ...
        it('should ignore system grep output that escapes base path', async () => {
            vi.mocked(execStreaming).mockImplementationOnce(() => createLineGenerator(['..env:1:hello', '../secret.txt:2:leak']));
            const params = { pattern: 'hello' };
            const invocation = grepTool.build(params);
            invocation.isCommandAvailable = vi.fn(async (command) => command === 'grep');
            const result = await invocation.execute(abortSignal);
            expect(result.llmContent).toContain('File: ..env');
    ...
    describe('getDescription', () => {
        ...
        it('should use ./ for root path in description', () => {
            const params = { pattern: 'testPattern', dir_path: '.' };
            const invocation = grepTool.build(params);
