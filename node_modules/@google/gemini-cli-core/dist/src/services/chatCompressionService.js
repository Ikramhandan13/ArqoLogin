...
import { calculateRequestTokenCount, estimateTokenCountSync, } from '../utils/tokenCalculation.js';
import { DEFAULT_GEMINI_FLASH_LITE_MODEL, DEFAULT_GEMINI_FLASH_MODEL, DEFAULT_GEMINI_MODEL, PREVIEW_GEMINI_MODEL, PREVIEW_GEMINI_FLASH_MODEL, PREVIEW_GEMINI_3_1_MODEL, } from '../config/models.js';
...
export function modelStringToModelConfigAlias(model) {
    switch (model) {
                ...
                if (part.functionResponse) {
                    ...
                    if (functionResponseTokenCounter + tokens >
                        ...
                        try {
                            ...
                            const { outputFile } = await saveTruncatedToolOutput(contentStr, part.functionResponse.name ?? 'unknown_tool', config.getNextCompressionTruncationId(), config.storage.getProjectTempDir());
                            const truncatedMessage = formatTruncatedToolOutput(contentStr, outputFile, config.getTruncateToolOutputThreshold());
                            newParts.unshift({
                                functionResponse: {
...
export class ChatCompressionService {
    async compress(chat, promptId, force, model, config, hasFailedCompressionAttempt, abortSignal) {
        ...
        const originalToCompressTokenCount = estimateTokenCountSync(originalHistoryToCompress.flatMap((c) => c.parts || []));
        const historyForSummarizer = originalToCompressTokenCount < tokenLimit(model)
            ? originalHistoryToCompress
            : historyToCompressTruncated;
        const hasPreviousSnapshot = historyForSummarizer.some((c) => c.parts?.some((p) => p.text?.includes('<state_snapshot>')));
        ...
        const newTokenCount = await calculateRequestTokenCount(fullNewHistory.flatMap((c) => c.parts || []), config.getContentGenerator(), model);
        logChatCompression(config, makeChatCompressionEvent({
            tokens_before: originalTokenCount,
