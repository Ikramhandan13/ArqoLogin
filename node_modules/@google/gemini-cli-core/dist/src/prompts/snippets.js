 ...
 */
import { ACTIVATE_SKILL_TOOL_NAME, ASK_USER_TOOL_NAME, EDIT_TOOL_NAME, ENTER_PLAN_MODE_TOOL_NAME, EXIT_PLAN_MODE_TOOL_NAME, GLOB_TOOL_NAME, GREP_TOOL_NAME, MEMORY_TOOL_NAME, READ_FILE_TOOL_NAME, SHELL_TOOL_NAME, WRITE_FILE_TOOL_NAME, WRITE_TODOS_TOOL_NAME, } from '../tools/tool-names.js';
 ...
 */
export function renderFinalShell(basePrompt, userMemory, contextFilenames) {
    return `
${basePrompt.trim()}
...
export function renderInteractiveYoloMode(enabled) {
    if (!enabled)
        return '';
    return `
...
export function renderUserMemory(memory, contextFilenames) {
    if (!memory)
        return '';
...
export function renderPlanningWorkflow(options) {
    if (!options)
        return '';
    return `
...
3. **Inquiries and Directives:** Distinguish between Inquiries and Directives to minimize unnecessary planning.
   - **Inquiries:** If the request is an **Inquiry** (e.g., "How does X work?"), use read-only tools to explore and answer directly in your chat response. DO NOT create a plan or call ${formatToolName(EXIT_PLAN_MODE_TOOL_NAME)}.
