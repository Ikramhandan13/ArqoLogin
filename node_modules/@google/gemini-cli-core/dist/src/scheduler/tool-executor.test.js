...
describe('ToolExecutor', () => {
    ...
    it('should truncate large shell output', async () => {
        ...
        expect(fileUtils.saveTruncatedToolOutput).toHaveBeenCalledWith(longOutput, SHELL_TOOL_NAME, 'call-trunc', expect.any(String), // temp dir
        'test-session-id');
        expect(fileUtils.formatTruncatedToolOutput).toHaveBeenCalledWith(longOutput, '/tmp/truncated_output.txt', 10);
        expect(result.status).toBe(CoreToolCallStatus.Success);
        if (result.status === CoreToolCallStatus.Success) {
            const response = result.response.responseParts[0]?.functionResponse
                ?.response;
    ...
    it('should report PID updates for shell tools', async () => {
        ...
        const testPid = 12345;
        vi.mocked(coreToolHookTriggers.executeToolWithHooks).mockImplementation(async (_inv, _name, _sig, _tool, _liveCb, _shellCfg, setPidCallback) => {
